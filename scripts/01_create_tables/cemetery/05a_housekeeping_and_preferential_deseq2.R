...
housekeeping <- read_tsv('../../tables/GTEx_expression_our_new_isoforms/deseq_input_housekeeping_genes.tsv')

preferential <- read_tsv('../../tables/GTEx_expression_our_new_isoforms/deseq_input_preferential_genes.tsv')
...
for (thresh in c(0.01, 1.01, 5.01, 10.01)) {
	...
	combined <- bind_rows(housekeeping, preferential) %>%
#	housekeeping_deseq2_input <- housekeeping %>%
		filter(threshold == thresh) %>%
		select(gene_id, transcript_id, gene_name) %>%
		left_join(gtex_counts %>% 
			  rename(transcript_id = TXNAME) %>% 
			  rename(gene_id = GENEID)) %>%
		unite('id', c(transcript_id, gene_id, gene_name))
#	print(housekeeping_deseq2_input)
	print(combined)
	...
	write_tsv(all_the_results %>%
		  #arrange(TXNAME), paste0('../../tables/GTEx_expression_our_new_isoforms/deseq_output_threshold_', thresh, '_housekeeping.tsv'))
		  arrange(TXNAME), paste0('../../tables/GTEx_expression_our_new_isoforms/deseq_output_threshold_', thresh, '_housekeeping_and_preferential.tsv'))
...
for (thresh in c(0.01, 1.01, 5.01, 10.01)) {
	preferential_deseq2_input <- preferential %>%
		filter(threshold == thresh) %>%
		select(gene_id, transcript_id, gene_name) %>%
		left_join(gtex_counts %>% 
			  rename(transcript_id = TXNAME) %>% 
			  rename(gene_id = GENEID)) %>%
		unite('id', c(transcript_id, gene_id, gene_name))
	print(preferential_deseq2_input)
	# start deseq2
	p_counts <- as.data.frame(preferential_deseq2_input)
	# pass the dataframe to deseq2
	dds <- DESeqDataSetFromMatrix(countData = p_counts,
				      colData = gtex_metadata,
				      design=~tissue_site_detail, tidy=TRUE)
	print(dds)
	dds <- DESeq(dds)
	...
	write_tsv(all_the_results %>%
		  arrange(TXNAME), paste0('../../tables/GTEx_expression_our_new_isoforms/deseq_output_threshold_', thresh, '_preferential.tsv'))
